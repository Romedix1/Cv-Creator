...
export default function ModernBlue({ data }: { data: ResumeData }) {
                ...
                <div key={cat.id} className="w-full flex flex-col gap-3">
                  ...
                  <ul className="flex flex-col gap-2">
                    {cat.skills.map((skill) => (
                      <li
                        key={skill.id}
                        className="bg-white px-2 py-1 rounded-[6px] wrap-break-word"
                      >
                        {skill.name}
                      </li>
                    ))}
        ...
        <div className="text-[11px] w-full">
          <ul className="flex flex-col gap-2 w-full">
            {data.languages.map((language) => (
              <li
                key={language.id}
                className="bg-white px-2 py-1 rounded-[6px] wrap-break-word w-full"
              >
                {language.value} -{" "}
  ...
  return (
    <div className={cn("bg-white flex", inter.className, "w-full text-black")}>
      {/* LEFT */}
      <div className="p-6 bg-[#EFF6FF] w-50 shrink-0 flex flex-col items-center gap-8 min-h-[297mm] print:w-full print:shadow-none print:mx-0">
        ...
        {(data.personalInfo.firstName ||
          ...
          <div className="mb-6">
            {(data.personalInfo.firstName || data.personalInfo.lastName) && (
              <h1 className="font-bold text-3xl wrap-break-word">
                {data.personalInfo.firstName}
                {data.personalInfo.lastName && ` ${data.personalInfo.lastName}`}
              </h1>
            )}
            {data.personalInfo.jobTitle && (
              <p className="font-medium text-[#2563EB] text-[16px] break-all">
                {data.personalInfo.jobTitle}
              </p>
            )}
          </div>
