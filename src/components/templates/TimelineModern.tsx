                  ...
                  <ul className="flex flex-col gap-3">
                    {cat.skills.map((skill) => (
                      <li
                        key={skill.id}
                        className={cn(
                          "font-medium wrap-break-word px-2 flex items-center gap-2",
                          spaceGrotesk.className,
                        )}
                      >
                        <span>
              ...
              {data.skillsCat
                ...
                .map((skill) => (
                  <li
                    key={skill.id}
                    className={cn(
                      "font-medium wrap-break-word flex items-center gap-2",
                      spaceGrotesk.className,
                    )}
                  >
                    <span>
          ...
          <ul className="flex flex-col gap-3">
            {data.languages.map((language) => (
              <li
                key={language.id}
                className={cn(
                  "px-2 wrap-break-word flex items-center gap-2 min-w-0",
                  spaceGrotesk.className,
                )}
              >
                <span>
                ...
                <div className="flex-1 min-w-0 wrap-break-word">
                  <span className="font-bold">{language.value}</span> -{" "}
                  {language.level}
                </div>
              </li>
            ))}
                    ...
                    <div className="flex flex-col flex-1 gap-1 min-w-0">
                      <div>
                        <div className="flex justify-between items-baseline">
                          ...
                          <span
                            className={cn(
                              "text-[10px] text-text-muted font-medium whitespace-nowrap flex-1",
                              spaceGrotesk.className,
                            )}
                          >
                            {item.startDate} - {item.endDate}
              ...
              {section.items.map((item, index) => {
                ...
                return (
                  <div key={item.id} className="flex">
                    ...
                    <div
                      key={item.id}
                      className="flex flex-col flex-1 gap-2.5 min-w-0"
                    >
                      <div>
                        <div className="flex justify-between items-baseline">
                          <h3 className="font-bold text-[13px] text-black wrap-break-word flex-1 min-w-0">
                        ...
                        >
                          ...
                          <ul className="flex flex-col gap-3">
                            {secItem.elements?.map((element) => (
                              <li
                                key={element.id}
                                className="px-2 wrap-break-word flex items-center gap-2 min-w-0"
                              >
                                <span>
                                  <Check className="shrink-0" />
                                </span>
      ...
      <div className="p-6 bg-[#F3F4F6] w-50 shrink-0 flex flex-col items-center gap-8 min-h-210.5">
        {data.personalInfo.avatarUrl && (
          <AppImage
            ...
            height={400}
            quality={100}
            className="rounded-[12px] w-25 h-25 object-cover ring-4 ring-white"
          />
        )}
        ...
        {(data.personalInfo.firstName ||
          ...
          <div className="mb-6">
            ...
            {data.personalInfo.jobTitle && (
              <p
                className={cn(
                  "text-[14px] break-all text-[#2563EB] font-bold",
                  spaceGrotesk.className,
                )}
              >
                {data.personalInfo.jobTitle}
