        ...
        <>
          {showContactSection && (
            <ul className="text-[10px] gap-2 flex justify-center">
              {contactDetails.map(
                (item: { id: string; value: string }, index) => (
                  <li
                    key={item.id}
              ...
              {data.personalInfo.links.map((link, index) => {
                return (
                  ...
                  >
                    ...
                    {index < (data.personalInfo.links?.length || 0) - 1 && (
                      <Dot className="stroke-2" />
                    )}
                  </li>
                );
        ...
        <div className="text-[11px] w-full">
          <ul className="flex flex-col gap-2">
            {data.languages.map((language) => (
              <li key={language.id} className="wrap-break-word">
                {language.value} -{" "}
  ...
  data.customSection?.forEach((section) => {
    if (section.items.length > 0) {
      sectionsMap[section.id] = {
        ...
        content: (() => {
          return section.layout === "center" && section.type === "detailed" ? (
            <div className="flex flex-col gap-4">
  ...
  return (
    ...
    >
      ...
      <div className="relative flex justify-center items-start w-full min-h-33.25">
        <div className="flex flex-col gap-2.5 items-center text-center">
          {(data.personalInfo.firstName || data.personalInfo.lastName) && (
            <h1 className="font-bold text-[32px] uppercase wrap-break-word max-w-61.75">
              {data.personalInfo.firstName}
              {data.personalInfo.lastName && ` ${data.personalInfo.lastName}`}
            </h1>
          )}
          {data.personalInfo.jobTitle && (
            <p className="font-medium text-[#374151] italic text-[14px] break-all">
              {data.personalInfo.jobTitle}
            </p>
          )}
        ...
        {data.personalInfo.avatarUrl && (
          <div className="absolute right-0 top-0 shrink-0">
            <AppImage
              ...
              containerClassName="w-25 h-33.25 overflow-hidden"
            />
          </div>
        )}
      </div>
      ...
      {sectionsToRender.map((section, index) => {
        ...
        return (
          <div key={index}>
            {section.id !== "contact" && section.position !== "left" && (
