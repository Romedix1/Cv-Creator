...
export default function SwissMinimalist({ data }: { data: ResumeData }) {
    ...
    const SectionHeader = (number: number, text: string) => {
        return (
            <div className="w-50 shrink-0 pr-6">
                <h2 className="text-[16px] font-bold flex"><span className="shrink-0">{number.toString().padStart(2, '0')}.</span>  <span className="w-full wrap-break-word [word-break:break-word]">{text}</span></h2>
            </div>
        )
    }
    ...
    const ItemHeader = (text: string) => {
        return (
            <h3 className="font-bold text-[14px] text-black wrap-break-word flex-1 min-w-0">{text}</h3>
        )
    }
    ...
    const sectionsMap: Record<string, { title: string, isVisible: boolean, content: React.ReactNode, position?: "left" | "center" }> = {
        "contact": {
            ...
            content: (
                <ul className="text-[12px] flex flex-col gap-2.5">
                    {data.personalInfo.phone && <li className="break-all">{data.personalInfo.phone}</li>}
                    {data.personalInfo.email && <li className="break-all">{data.personalInfo.email}</li>}
                    {data.personalInfo.address && <li className="break-all">{data.personalInfo.address}</li>}
                    {data.personalInfo.links?.map((link) => {
                        return (
                            <li key={link.id}><Link href={link.url} className="break-all underline">{link.url}</Link></li>
                        )
                    })}
                </ul>
            )
                            ...
                            <div key={item.id} className="flex flex-col gap-2.5">
                                <div>
                                    <div className="flex justify-between items-baseline">
                                        ...
                                        {ItemDate(`${item.startDate} ${item.endDate && "-"} ${item.endDate}`)}
                                    </div>
            ...
            content: (
                <div className="text-[11px] w-full">
                    {data.settings.skillsType === "categories" ? (
                                ...
                                <div key={cat.id} className="w-full flex flex-col gap-3">
                                    ...
                                    <ul className="flex gap-2">
                                        {cat.skills.map((skill, index) => (
                                            <li key={skill.id} className="wrap-break-word">{skill.name} {index+1 !== cat.skills.length && ","}</li>
                                        ))}
                                    </ul>
                                </div>
                            ))}
                        ...
                        <ul className="flex gap-2">
                            {data.skillsCat.flatMap(cat => cat.skills).map((skill, index) => (
                                <li key={skill.id} className="wrap-break-word">{skill.name} {index+1 !== data.skillsCat.length && ","}</li>
                            ))}
                        </ul>
                    )}
                </div>
        ...
        "languages": {
            ...
            content: (() => {
                ...
                const fluentLangs = data.languages.filter(lang => fluentLevels.includes(lang.level))
                const basicLangs = data.languages.filter(lang => !fluentLevels.includes(lang.level))
                            ...
                            <li>
                                ...
                                <ul>
                                    {fluentLangs.map((lang, index) => (
                                        <li key={lang.id}>{lang.value} {"("+lang.level+")"}{index+1 !== fluentLangs.length && ","}</li>
                                    ))}
                                </ul>
                            </li>
                            <li>
                                ...
                                <ul>
                                    {basicLangs.map((lang, index) => (
                                        <li key={lang.id}>{lang.value} {"("+lang.level+")"}{index+1 !== basicLangs.length && ","}</li>
                                    ))}
                                </ul>
                            </li>
                        </ul>
        ...
        <div className={cn("bg-white flex flex-col gap-8", inter.className, "w-full text-black p-10 min-h-[297mm]")}>
            ...
            <div className="relative flex w-full min-h-33.25">
                <div className="flex flex-col w-9/12">
                    {(data.personalInfo.firstName || data.personalInfo.lastName) && <h1 className="font-extrabold text-[42px] uppercase wrap-break-word leading-9 ">{data.personalInfo.firstName}<br />{data.personalInfo.lastName && ` ${data.personalInfo.lastName}`}</h1>}
                    {data.personalInfo.jobTitle && <p className="font-extrabold text-center text-[14px] break-all">{data.personalInfo.jobTitle}</p>}
                </div>
