...
export default function Template({ templateId, name, image, onPreview, enablePreview = true }: TemplateProps) {
        ...
        <article className="group px-2.5 py-2.5 bg-surface border rounded-2xl lg:w-87.5 2xl:w-112.5">
            <div className="relative overflow-y-hidden h-95 sm:h-200 lg:h-120 2xl:h-135">
                ...
                <div className="hidden lg:flex absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 duration-300 flex-col items-center justify-center px-6 gap-4">
                    <Button variant="primary" text={tButton("select")} className="w-full" href={`/cv-builder/${newResumeId}?template=${templateId}`}/>
            ...
            <div className="flex flex-col items-center px-4 relative">
                ...
                <Button variant="primary" text={tButton("select")} className="w-full my-2 lg:hidden" href={`/cv-builder/${newResumeId}?template=${templateId}`} />
