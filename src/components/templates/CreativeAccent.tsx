...
export default function CreativeAccent({ data }: { data: ResumeData }) {
  ...
  const SectionIcon = (id: string, title: string) => {
    ...
    if (!Icon) {
      ...
      const match = (keywords: string[]) =>
        keywords.some((key) => sectionTitle.includes(key));
      ...
      if (
        ...
        match(["publika", "publicat", "artykuł", "article", "paper"])
      ) {
        Icon = AtSign;
      } else {
        Icon = Layers;
    ...
    contact: {
      ...
      content: (
        <ul className="text-[10px] gap-2 flex flex-col">
          {data.personalInfo.phone && (
            <li className="flex gap-2 items-center break-all">
          ...
          {data.personalInfo.links?.map((link) => {
            return (
              <li key={link.id} className="flex gap-2 items-center break-all">
                <span className="shrink-0">
    ...
    profile: {
      ...
      content: (
        <div className="flex">
          ...
          <p className="text-[11px] leading-3.75 w-full wrap-break-word">
              ...
              <div key={item.id} className="flex">
                ...
                <div key={item.id} className="flex flex-col flex-1 gap-2.5">
                  <div>
                    ...
                    <p
                      className="text-[11px] font-semibold mb-1 wrap-break-word flex-1 min-w-0"
          ...
          {data.settings.skillsType === "categories" ? (
                ...
                <div key={cat.id} className="w-full flex flex-col gap-3">
                  ...
                  <ul className="flex flex-col gap-2 list-disc px-6">
                    {cat.skills.map((skill) => (
                      <li
                        key={skill.id}
                        className="rounded-[6px] wrap-break-word"
                      >
                        {skill.name}
                      </li>
                    ))}
            ...
            <ul className="flex flex-col gap-2 px-6">
              {data.skillsCat
                ...
                .map((skill) => (
                  <li key={skill.id} className="rounded-[6px] wrap-break-word">
                    {skill.name}
                  </li>
                ))}
            </ul>
        ...
        <div className="text-[11px] w-full">
          <ul className="flex flex-1 flex-col gap-2 list-disc px-3.5">
            {data.languages.map((language) => (
              <li key={language.id} className="rounded-[6px] wrap-break-word">
                {language.value} -{" "}
  ...
  return (
    ...
    >
      {/* TOP */}
      <div
        className="flex w-full py-11 relative"
      ...
      >
        {data.personalInfo.avatarUrl && (
          <AppImage
            ...
            containerClassName="rounded-full w-30 h-30 object-cover absolute ring-4 ring-white left-6 bottom-0 translate-y-1/2"
          />
        )}
        ...
        {(data.personalInfo.firstName ||
          ...
          <div className="text-left w-full pl-42">
            {(data.personalInfo.firstName || data.personalInfo.lastName) && (
              <h1 className="font-bold text-3xl wrap-break-word text-white">
                {data.personalInfo.firstName}
                {data.personalInfo.lastName && ` ${data.personalInfo.lastName}`}
              </h1>
            )}
            {data.personalInfo.jobTitle && (
              <p className="font-medium text-[16px] text-white break-all">
                {data.personalInfo.jobTitle}
              </p>
            )}
          </div>
          ...
          {sectionsToRender.map((section, index) => {
            ...
            if (section.position === "left") {
              return (
                <div
                  key={`left-${index}`}
                  className="flex flex-col gap-3 w-full wrap-break-word"
                >
              ...
              if (section.position === "center") {
                return (
                  <div key={`center-${index}`}>
                    ...
                    <div className="ml-2 flex-1 min-w-0">{section.content}</div>
                  </div>
                );
              }
            })}
