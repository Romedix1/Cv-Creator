...
export default function ResumeEditor({
  ...
  const STEPS = [
    {
      key: "personalData",
      name: tCvBuilderSteps("personalData"),
      icon: <User2 className={iconStyles} />,
    },
    {
      key: "experience",
      name: tCvBuilderSteps("experience"),
      icon: <BriefcaseBusiness className={iconStyles} />,
    },
    {
      key: "education",
      name: tCvBuilderSteps("education"),
      icon: <GraduationCap className={iconStyles} />,
    },
    {
      key: "skills",
      name: tCvBuilderSteps("skills"),
      icon: <Wrench className={iconStyles} />,
    },
    {
      key: "languages",
      name: tCvBuilderSteps("languages"),
      icon: <Languages className={iconStyles} />,
    },
    {
      key: "certificates",
      name: tCvBuilderSteps("certificates"),
      icon: <Award className={iconStyles} />,
    },
    {
      key: "interests",
      name: tCvBuilderSteps("interests"),
      icon: <Heart className={iconStyles} />,
    },
    {
      key: "rodo",
      name: tCvBuilderSteps("rodo"),
      icon: <ShieldCheck className={iconStyles} />,
    },
    {
      key: "summary",
      name: tCvBuilderSteps("summary"),
      icon: <FileText className={iconStyles} />,
    },
    {
      key: "addSection",
      name: tCvBuilderSteps("addSection"),
      ...
      if (!isAuthenticated) {
        ...
        try {
          ...
          const { error } = await supabase.from("resumes").upsert({
            ...
            template: debouncedData.settings.template,
            title: debouncedTitle || tBuilderNav("documentName"),
            user_id: user.id,
  ...
  useEffect(() => {
    const uploadImage = async () => {
      if (!selectedFile || typeof selectedFile === "string" || !resumeId)
        return;
      ...
      if (!isAuthenticated) {
        ...
        reader.onloadend = () => {
          ...
          setData((prev) => ({
            ...prev,
            personalInfo: { ...prev.personalInfo, avatarUrl: base64String },
          }));
      ...
      try {
        ...
        setData((prev) => ({
          ...prev,
          personalInfo: { ...prev.personalInfo, avatarUrl: freshUrl },
        }));
      } catch {
  ...
  const displaySection = (currentStep: string) => {
    switch (currentStep) {
      case "personalData":
        return (
          <PersonalDataSection
            ...
            onPersonalInfoChange={(newItems) =>
              handleSectionChange("personalInfo", newItems)
            }
      ...
      case "experience":
        return (
          <ExperienceSection
            ...
            onExperienceChange={(newItems) =>
              handleSectionChange("experience", newItems)
            }
      ...
      case "education":
        return (
          <EducationSection
            ...
            onEducationChange={(newItems) =>
              handleSectionChange("education", newItems)
            }
            setIsEditingMode={setIsEditingMode}
          />
        );
      case "skills":
        return (
          <SkillsSection
            ...
            categories={data.skillsCat}
            viewMode={viewMode}
            setViewMode={setViewMode}
            onSkillsChange={(newItems) =>
              handleSectionChange("skillsCat", newItems)
            }
            onSettingsChange={(newValue) =>
              handleSectionChange("settings", newValue)
            }
          />
        );
      case "languages":
        return (
          <LanguagesSection
            ...
            onLanguageChange={(newItems) =>
              handleSectionChange("languages", newItems)
            }
            setIsEditingMode={setIsEditingMode}
          />
        );
      case "certificates":
        return (
          <CertificatesSection
            ...
            onCertificatesChange={(newItems) =>
              handleSectionChange("certificates", newItems)
            }
            setIsEditingMode={setIsEditingMode}
          />
        );
      case "interests":
        return (
          <InterestsSection
            ...
            onCertificatesChange={(newItems) =>
              handleSectionChange("interests", newItems)
            }
            setIsEditingMode={setIsEditingMode}
          />
        );
      case "rodo":
        return (
          <RodoSection
            rodo={data.rodoSection}
            onRodoChange={(newItems: RodoSectionType[]) =>
              handleSectionChange("rodoSection", newItems)
            }
          />
        );
      case "summary":
        const syncedOrder = Array.from(
          new Set([
            ...data.settings.sectionOrder,
            ...data.customSection.map((s) => s.id),
          ]),
        );

        return (
          <SummarySection
            sections={syncedOrder}
            onSectionsOrderChange={(newOrder: string[]) =>
              handleSectionChange("settings", {
                ...data.settings,
                sectionOrder: newOrder,
              })
            }
            customSections={data.customSection}
            onTemplateChange={(newValue) =>
              handleSettingChange("template", newValue)
            }
            onColorChange={(newValue) => handleSettingChange("color", newValue)}
      ...
      case "addSection":
        return (
          <CustomSection
            ...
            onSectionChange={(newItems: CustomSectionType[]) =>
              handleSectionChange("customSection", newItems)
            }
            setIsEditingMode={setIsEditingMode}
          />
        );
        ...
        {!isEditingMode && (
          <div className="w-full grid grid-cols-2 gap-8 mt-16 px-3 sm:px-12">
            <Button
              ...
              className={cn(
                "w-full",
                STEPS.findIndex((link) => link.key === currentStep) === 0
                  ? "invisible pointer-events-none transition-none"
                  : "",
              )}
              text={tButton("prevBtn")}
              onClick={() => handlePrevButton(currentStep)}
            />
            <Button
              variant="primary"
              className="w-full"
              text={tButton("nextBtn")}
              onClick={() => handleNextButton(currentStep)}
            />
          </div>
        )}
      </div>
